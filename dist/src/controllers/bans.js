...
async function getBans(req, res, next) {
    try {
...
async function addBan(req, res, next) {
    try {
        // Validate IP address.
...
async function deleteBan(req, res, next) {
    try {
        const banId = helpers.validateBanId(req.params.banId);
        // Find the ban.
        const ban = await globals_1.prisma.ban.findUnique({
            where: {
                id: banId,
            },
        });
