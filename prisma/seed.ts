...
const boards = [
  { id: 'b', title: 'Random' },
  { id: 'k', title: 'Weapons' },
  { id: 'pol', title: 'Politics' },
  { id: 'v', title: 'Video Games' },
  { id: 'x', title: 'Paranormal' },
];
...
async function seedBoards() {
  for (const { id, title } of boards) {
    ...
    await prisma.role.createMany({
      data: [
        {
          id: `${id}:${PermissionLevel.ADMIN}`,
          boardId: id,
          level: PermissionLevel.ADMIN,
        },
        {
          id: `${id}:${PermissionLevel.MODERATOR}`,
          boardId: id,
          level: PermissionLevel.MODERATOR,
        },
        {
          id: `${id}:${PermissionLevel.JANITOR}`,
          boardId: id,
...
async function seedThreads() {
  for (const { id: boardId } of boards) {
    // Seed sticky threads.
    for (let i = 0; i < 3; i++) {
      await seedThread({
...
async function seedThread(params: any) {
  ...
  // Create first reply with all available formatting modes.
  ...
  for (const user of users) {
    ...
    await prisma.mod.create({
      data: {
        ...
        password,
        roles: {
          connect: boards.map(({ id }) => {
            return { id: `${id}:${user.permissionLevel}` };
          }),
        },
      },
    });
    console.log(`Seeded mod ${user.username} (${user.email})`);
  }
}
